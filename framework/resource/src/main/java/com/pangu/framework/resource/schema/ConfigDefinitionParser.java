...
import static com.pangu.framework.resource.schema.SchemaNames.CLASS_ATTRIBUTE_NAME;
import static com.pangu.framework.resource.schema.SchemaNames.PACKAGE_ATTRIBUTE_NAME;
...
public class ConfigDefinitionParser extends AbstractBeanDefinitionParser {
	...
	private FormatDefinition parseFormat(Element element) {
		...
		for (int i = 0; i < child.getLength(); i++) {
			...
			String n18 = e.hasAttribute(SchemaNames.FORMAT_ATTRIBUTE_I18N) ? e
					.getAttribute(SchemaNames.FORMAT_ATTRIBUTE_I18N) : "false";
			String config = e.hasAttribute(SchemaNames.FORMAT_ATTRIBUTE_CONFIG) ? e
					.getAttribute(SchemaNames.FORMAT_ATTRIBUTE_CONFIG) : null;
			return new FormatDefinition(location, type, n18, suffix, config);
		}
