...
import com.dianping.cat.configuration.CustomProperties;
...
public class Cat {
    ...
    public static boolean isJstackEnabled() {
        ...
        return JSTACK_ENABLED && Boolean.parseBoolean(enable);
    }
    ...
    private static void customInit() {
        if (!CustomProperties.isEnable()) {
            return;
        }

        String model = CustomProperties.getName();
        if (UNKNOWN.equals(model)) {
            init = true;
            enabled = false;
            return;
        }
        String servers = CustomProperties.getServers();
        if (servers == null || servers.trim().isEmpty()) {
            init = true;
            enabled = false;
            return;
        }
        String[] mulServers = servers.split(",");
        ClientConfig config = new ClientConfig();

        config.setDomain(ApplicationEnvironment.loadAppName(model));

        for (String serverItem : mulServers) {
            if (serverItem == null) {
                continue;
            }
            serverItem = serverItem.trim();
            String[] addr = serverItem.split(":");
            if (addr.length == 0) {
                continue;
            }
            String ip = addr[0];
            ...
            int httpPort = 8080;
            if (addr.length >= 3) {
                httpPort = Integer.parseInt(addr[2]);
            }
            server.setPort(port);
