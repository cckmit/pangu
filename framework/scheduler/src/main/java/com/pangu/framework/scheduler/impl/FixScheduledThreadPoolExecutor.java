...
public class FixScheduledThreadPoolExecutor extends FixThreadPoolExecutor implements ScheduledExecutorService {
	...
	public FixScheduledThreadPoolExecutor(Integer poolSize, Long delayTime, ThreadFactory threadFactory,
			boolean startupImmediately) {
		super(poolSize, Integer.MAX_VALUE, 0, TimeUnit.NANOSECONDS, new DelayedWorkQueue(delayTime), threadFactory);
		this.startupImmediately = startupImmediately;
		if (!startupImmediately) {
			waitingDelayedTasks = new LinkedBlockingQueue<Runnable>();
		}
	}
	...
	/**
	 * startup tasks immediately
	 */
	private volatile boolean startupImmediately = true;

	/**
	 * waiting Delayed Tasks while not startup
	 */
	private volatile LinkedBlockingQueue<Runnable> waitingDelayedTasks;
	 ...
	 */
	public void startUp() {
		this.startupImmediately = true;
		while (!waitingDelayedTasks.isEmpty()) {
			Runnable poll = waitingDelayedTasks.poll();
			if (poll != null) {
				delayedExecute(poll);
			}
		}
	...
	private void delayedExecute(Runnable command) {
		...
		if (!startupImmediately) {
			// 未启动
			waitingDelayedTasks.add(command);
			return;
		}
	...
	public ScheduledFuture<?> schedule(Runnable command, long delay, TimeUnit unit) {
		...
		RunnableScheduledFuture<?> t = decorateTask(command, new ScheduledFutureTask<Boolean>(command, null,
				triggerTime));
		delayedExecute(t);
		return t;
	}
	...
	public ScheduledFuture<?> scheduleAtFixedRate(Runnable command, long initialDelay, long period, TimeUnit unit) {
		...
		RunnableScheduledFuture<?> t = decorateTask(command, new ScheduledFutureTask<Object>(command, null,
				triggerTime, unit.toMillis(period)));
		delayedExecute(t);
		return t;
	}
	...
	public ScheduledFuture<?> scheduleWithFixedDelay(Runnable command, long initialDelay, long delay, TimeUnit unit) {
		...
		long triggerTime = now() + unit.toMillis(initialDelay);
		RunnableScheduledFuture<?> t = decorateTask(command, new ScheduledFutureTask<Boolean>(command, null,
				triggerTime, unit.toMillis(-delay)));
		delayedExecute(t);
		return t;
	}
	 ...
	 * Note that inspections of the queue and of the list returned by <tt>shutdownNow</tt> will access the zero-delayed
	 ...
	 * <tt>shutdown</tt>. In this case, these tasks will only terminate upon <tt>shutdownNow</tt>, or after setting the
	 * policy to <tt>false</tt> when already shutdown. This value is by default false.
	...
	/**
	 * Sets the policy on whether to execute existing delayed tasks even when this executor has been <tt>shutdown</tt>.
	 * In this case, these tasks will only terminate upon <tt>shutdownNow</tt>, or after setting the policy to
	...
	/**
	 * Gets the policy on whether to execute existing delayed tasks even when this executor has been <tt>shutdown</tt>.
	 * In this case, these tasks will only terminate upon <tt>shutdownNow</tt>, or after setting the policy to
	 * <tt>false</tt> when already shutdown. This value is by default true.
	 ...
	 * accepted. If the <tt>ExecuteExistingDelayedTasksAfterShutdownPolicy</tt> has been set <tt>false</tt>, existing
	 * delayed tasks whose delays have not yet elapsed are cancelled. And unless the
	 * <tt>ContinueExistingPeriodicTasksAfterShutdownPolicy</tt> has been set <tt>true</tt>, future executions of
	 * existing periodic tasks will be cancelled.
	 */
	 ...
	 * never terminate.
	 * @return list of tasks that never commenced execution. Each element of this list is a {@link ScheduledFuture},
	 *         including those tasks submitted using <tt>execute</tt>, which are for scheduling purposes used as the
	 ...
	 * those tasks submitted using <tt>execute</tt> which are for scheduling purposes used as the basis of a zero-delay
	 * <tt>ScheduledFuture</tt>. Iteration over this queue is <em>not</em> guaranteed to traverse tasks in the order in
	...
	/**
	 * An annoying wrapper class to convince javac to use a DelayQueue<RunnableScheduledFuture> as a
