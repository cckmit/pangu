...
import com.pangu.framework.utils.math.MathUtils;
...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
    ...
    /**
     * 日期转换成字符串格式（使用格式：yyyy-MM-dd HH:mm:ss）
     *
     * @param date 日期
     */
    public static String date2String(Date date) {
        return date2String(date, PATTERN_DATE_TIME);
    }
     ...
     * timestame转换成字符串格式
     *
     * @param timestame 待转换的日期
     * @param pattern   日期格式
     * @return {@link String} 日期字符串
     */
    public static String time2String(long timestame, String pattern) {
        return new SimpleDateFormat(pattern).format(new Date(timestame));
    }
    ...
    /**
     * 计算2个时间相差的秒数,这个方法算的是2个零点时间的绝对时间
     *
     * @param start 起始时间
     * @param end   结束时间
     * @return 相差的秒数
     */
    public static int calcIntervalSecend(Date start, Date end) {
        return Math.abs((int) Math.ceil((start.getTime() - end.getTime()) / 1000));
    }
    ...
    public static Date getTimeBefore(String cron, Date time) {
        return CronUtils.getTimeBefore(cron, time);
    }
