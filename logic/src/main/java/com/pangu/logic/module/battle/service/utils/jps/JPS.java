...
public abstract class JPS {
    ...
    public Future<Queue<Node>> findPath(Node start, Node goal) {
        return findPath(start, goal, false, false);
    }

    public Future<Queue<Node>> findPath(Node start, Node goal, boolean adjacentStop) {
        return findPath(start, goal, adjacentStop, true);
    }

    public Future<Queue<Node>> findPath(Node start, Node goal, boolean adjacentStop, boolean diagonalStop) {
        FutureTask<Queue<Node>> future = new FutureTask<>(() -> findPathSync(start, goal, adjacentStop, diagonalStop));
        future.run();
        return future;
    }

    public Queue<Node> findPathSync(Node start, Node goal) {
        return findPathSync(start, goal, false, false);
    }

    public Queue<Node> findPathSync(Node start, Node goal, boolean adjacentStop) {
        return findPathSync(start, goal, adjacentStop, true);
    }

    public Queue<Node> findPathSync(Node start, Node goal, boolean adjacentStop, boolean diagonalStop) {
        Map<Node, Double> fMap = new HashMap<>(); // distance to start + estimate to end
        Map<Node, Double> gMap = new HashMap<>(); // distance to start (parent's g + distance from parent)
        Map<Node, Double> hMap = new HashMap<>(); // estimate to end

        Queue<Node> open = new PriorityQueue<>((a, b) -> {
    ...
    private void identifySuccessors(Node node, Node goal, Set<Node> goals, Queue<Node> open, Set<Node> closed, Map<Node, Node> parentMap,
                                    Map<Node, Double> fMap, Map<Node, Double> gMap, Map<Node, Double> hMap) {
     ...
     */
    protected abstract Set<Node> findNeighbors(Node node, Map<Node, Node> parentMap);

    /**
     * Search towards the child from the parent, returning when a jump point is found.
     */
    protected abstract Node jump(Node neighbor, Node current, Set<Node> goals);
    ...
    public static class JPSFactory {
        public static JPS getJPS(Graph graph, Graph.Diagonal diagonal) {
            switch (diagonal) {
                case ALWAYS:
                ...
                default:
                    throw new IllegalStateException("不支持" + diagonal + "");
            }
        }
    }
}
